...
export async function GET() {
  ...
  type Rec = { source: Source; name: string; email: string; detail: string; paid: boolean };
  ...
  return NextResponse.json({
    ...
    totals: group("attendee").payments + group("sponsor").payments + group("unattributed").payments
      ? {
          payments: lines.length,
          grossCents: lines.reduce((t, l) => t + l.grossCents, 0),
          feeCents: lines.reduce((t, l) => t + l.feeCents, 0),
          refundedCents: lines.reduce((t, l) => t + l.refundedCents, 0),
          netCents: lines.reduce((t, l) => t + l.netCents, 0),
        }
      : { payments: 0, grossCents: 0, feeCents: 0, refundedCents: 0, netCents: 0 },
