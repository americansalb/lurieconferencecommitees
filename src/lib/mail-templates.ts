...
// The guide emails.
//
// These go to people who have already paid and are coming, so the letter has
// no selling to do. It gets to the point: what is attached, where they stand,
// and the one thing only they can confirm.
//
// Deliberately no letterhead. The engraved seal and the teal hero block belong
// on an invitation, where the first job is to impress; here they push the
// actual information below the fold on a phone. A single quiet header line
...
function guideShell(args: {
  ...
  const postal = process.env.MAIL_POSTAL_ADDRESS?.trim() || "Americans Against Language Barriers, Chicago, IL";
  return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
...
<style>
  ...
  a{color:${G.TEAL};}
  @media only screen and (max-width:600px){
    .g-card{width:100%!important;}
    .g-pad{padding:26px 22px 28px 22px!important;}
    .g-head{padding:18px 22px!important;}
    .g-h1{font-size:21px!important;line-height:28px!important;}
    .g-cta{display:block!important;width:100%!important;}
  }
</style>
</head>
<body style="margin:0;padding:0;width:100%;background-color:#EEF2F4;">
<div style="display:none;max-height:0;overflow:hidden;mso-hide:all;font-size:1px;line-height:1px;color:#EEF2F4;">${args.preheader}</div>

<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color:#EEF2F4;">
<tr><td align="center" style="padding:22px 12px 30px 12px;">

  <table role="presentation" class="g-card" width="600" cellpadding="0" cellspacing="0" border="0" style="width:600px;max-width:600px;background-color:#FFFFFF;border:1px solid #E2E8EA;border-radius:12px;overflow:hidden;">

    <!-- One quiet line: hosts on the left, what this is on the right. -->
    <tr><td class="g-head" style="padding:18px 30px;border-bottom:1px solid ${G.RULE};">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr>
        <td align="left" valign="middle">
          <img src="${base}/logos/aalb.png" alt="Americans Against Language Barriers" height="22" style="height:22px;width:auto;display:inline-block;vertical-align:middle;">
          <img src="${base}/logos/lurie.png" alt="Ann &amp; Robert H. Lurie Children&rsquo;s Hospital of Chicago" height="18" style="height:18px;width:auto;display:inline-block;vertical-align:middle;padding-left:12px;">
        </td>
        <td align="right" valign="middle" style="font-family:Helvetica,Arial,sans-serif;font-size:9px;font-weight:bold;letter-spacing:2.2px;text-transform:uppercase;color:${G.GOLD};white-space:nowrap;">${args.kicker}</td>
      </tr></table>
    </td></tr>

    <tr><td class="g-pad" style="padding:32px 30px 34px 30px;">
      ${args.body}
    </td></tr>

    <tr><td style="padding:18px 30px 22px 30px;border-top:1px solid ${G.RULE};background-color:${G.SOFT_BG};">
      <div style="font-family:Helvetica,Arial,sans-serif;font-size:11px;line-height:1.6;color:#7C8B92;">${args.footerNote}</div>
      <div style="font-family:Helvetica,Arial,sans-serif;font-size:11px;line-height:1.6;color:#9AA7AD;padding-top:6px;">
        conference.aalb.org &middot; contact@aalb.org<br/>
        ${escapeHtml(postal)} &middot; 501(c)(3) &middot; EINs 83-3016421 and 36-2170833
      </div>
    </td></tr>
...
const gP = (html: string, mb = 16) =>
  `<p style="margin:0 0 ${mb}px 0;font-family:Helvetica,Arial,sans-serif;font-size:15px;line-height:1.65;color:${G.BODY};">${html}</p>`;

const gH1 = (html: string) =>
  `<h1 class="g-h1" style="margin:0 0 12px 0;font-family:Helvetica,Arial,sans-serif;font-size:24px;line-height:31px;font-weight:bold;color:${G.INK};letter-spacing:-0.2px;">${html}</h1>`;

const gLabel = (text: string) =>
  `<div style="font-family:Helvetica,Arial,sans-serif;font-size:9.5px;font-weight:bold;letter-spacing:2px;text-transform:uppercase;color:${G.GOLD};margin:24px 0 10px 0;">${text}</div>`;

// Facts as hairline-separated rows. No heavy boxes: the information is the
// point, and a stack of filled panels is what made this look like a brochure.
function gFacts(rows: { label: string; value: string }[], accent = false): string {
  return `
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 6px 0;${accent ? `background-color:${G.GOLD_BG};border:1px solid ${G.GOLD_RULE};border-radius:10px;` : ""}">
        ${rows.map((r, i) => `
        <tr><td style="padding:${i === 0 ? (accent ? "14px" : "0") : "12px"} ${accent ? "18px" : "0"} 12px ${accent ? "18px" : "0"};${i > 0 ? `border-top:1px solid ${accent ? G.GOLD_RULE : G.RULE};` : ""}">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr>
            <td width="150" valign="top" style="width:150px;font-family:Helvetica,Arial,sans-serif;font-size:12.5px;line-height:1.5;font-weight:bold;color:${G.INK};padding-right:14px;">${r.label}</td>
            <td valign="top" style="font-family:Helvetica,Arial,sans-serif;font-size:14px;line-height:1.55;color:${G.BODY};">${r.value}</td>
          </tr></table>
        </td></tr>`).join("")}
      </table>`;
}

function gButton(href: string, label: string): string {
  return `
      <table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin:24px 0 4px 0;">
        <tr><td align="center" bgcolor="${G.TEAL}" class="g-cta" style="background-color:${G.TEAL};border-radius:8px;">
...
const gSignOff = () => `
      <div style="margin-top:26px;padding-top:16px;border-top:1px solid ${G.RULE};font-family:Helvetica,Arial,sans-serif;font-size:13px;line-height:1.6;color:${G.BODY};">
        <span style="font-weight:bold;color:${G.INK};">AALB Conference Committee</span><br/>
...
export function attendeeGuideEmail({
  ...
  const facts = isVirtual
    ? [
        { label: "When", value: "Saturday, August 15 and Sunday, August 16" },
        { label: "Where", value: "Online. Your joining link arrives before the event." },
        { label: "CEUs", value: "Earned by attending live." },
      ]
    : [
        { label: "When", value: attendDay ? `${escapeHtml(attendDay)} only` : "Saturday, August 15 and Sunday, August 16" },
        { label: "Where", value: "Lurie Children&rsquo;s Hospital, 225 East Chicago Avenue, Chicago. 11th floor." },
        { label: "Check-in", value: "Saturday 9:00 to 9:30 AM, Sunday 8:30 to 9:00 AM. Photo ID both days." },
        { label: "Lunch", value: "Meat free, both days." },
        ...(fullName
          ? [{ label: "Your badge", value: escapeHtml(fullName) + (sponsorName ? `, guest of ${escapeHtml(sponsorName)}` : "") }]
          : []),
      ];

  const onFile = [
    ...(has(dietary) ? [{ label: "Dietary", value: escapeHtml(has(dietary)) }] : []),
    ...(has(accessibilityNotes) ? [{ label: "Access", value: escapeHtml(has(accessibilityNotes)) }] : []),
    ...(!isVirtual && needsParking ? [{ label: "Parking", value: "You asked about parking." }] : []),
  ...
  const body = `
      ...
      ${gLabel("The essentials")}
      ${gFacts(facts)}

      ${hasNeeds ? `
      ${gLabel("What we have recorded for you")}
      ${gFacts(onFile, true)}
      ${gP("We plan meals and seating from these answers. Tell us if any of it is wrong.", 0)}`
      : `
      ${gLabel("Anything you need from us")}
      ${gP("We have no dietary or access needs recorded for you. If you have any, add them in your portal and we will plan around them.", 0)}`}

      ${gButton(portalUrl, "Open my portal")}
      ${gP("Questions are welcome by reply.", 0)}
      ${gSignOff()}`;

  return guideShell({
    title: "Your conference guide",
    preheader: isVirtual
      ? "Joining details, the schedule, and how to claim your CEUs."
      : "Getting there, check-in times, meals, and how to claim your CEUs.",
...
export function exhibitorGuideEmail({
  ...
  const body = `
      ...
      ${gLabel("Your table")}
      ${gFacts([
        { label: "Load-in", value: "Friday, August 14, 4:30 to 8:00 PM, or Saturday 7:00 to 8:00 AM. Set up by 8:45 AM Saturday, after which nothing can be moved in or around the space." },
        { label: "Teardown", value: "Stay set up until 4:30 PM Sunday, August 16." },
        { label: "Before you travel", value: "Text Adriana at 773-573-0678 with your arrival time and someone will meet you." },
        { label: "Busiest", value: "Coffee breaks and lunch, both days." },
      ])}

      ${gLabel("Shipping ahead")}
      ${gFacts([
      ...
      ${gP(`Once it is on its way, email <a href="mailto:contact@aalb.org">contact@aalb.org</a> with an inventory and the address we should notify when it arrives.`, 0)}
      ...
      ${gFacts([
        { label: "On your list", value: roster },
        {
          ...
          value: seatsRemaining > 0
            ...
            : "All claimed. Anyone else can register at the usual rate through the same link.",
        },
      ])}

      ${gButton(teamUrl, "Your team page")}
      ${gP("Anything you need on the day, reply here.", 0)}
      ${gSignOff()}`;

  return guideShell({
    title: "Your exhibitor guide",
    preheader: `Load-in times, parking, shipping, and your team list for ${companyName}.`,
    kicker: "Exhibitor Guide",
    body,
    footerNote: `You are receiving this because ${escapeHtml(companyName)} is exhibiting at the 2026 Lurie Children&rsquo;s &amp; AALB Conference.`,
    assetBase,
  });
}
...
export function plainDirectInviteEmail(args: AttendeeInviteArgs) {
  ...
  // Unsubscribe link in the body. Nobody writing one person a real letter
  // signs off and then explains how they got the address.
  //
...
// Sent to a confirmed presenter: your seat is already booked, here is your
// attendee portal, please confirm the practical details. Same engraved letter
// as the sponsor correspondence, because a presenter is not a cold contact.
export function presenterAttendeeConfirmEmail({
  name,
  talkTitle,
  portalUrl,
  hasDetails,
  ...
  portalUrl: string;
  // True when their proposal already gave us dietary/accessibility answers, so
  // the letter asks them to check rather than to fill anything in.
  hasDetails: boolean;
  siteUrl?: string | null;
  dateLabel?: string;
  assetBase?: string;
  ...
  const p = (html: string, mb = 18) =>
    `<p style="margin:0 0 ${mb}px 0;font-family:Georgia,'Times New Roman',serif;font-size:15.5px;line-height:1.85;color:${INK};">${html}</p>`;
  const sig = (img: string, n: string, title: string) => `
        <img src="${base}/sig/${img}" alt="${escapeHtml(n)}" height="40" style="height:40px;width:auto;display:block;margin:0 0 4px 0;">
        <table role="presentation" cellpadding="0" cellspacing="0" border="0"><tr><td style="width:140px;height:1px;background-color:${GOLD};font-size:0;line-height:0;">&nbsp;</td></tr></table>
        <div style="font-family:Helvetica,Arial,sans-serif;font-size:11px;letter-spacing:1.4px;text-transform:uppercase;color:${INK};font-weight:bold;padding-top:8px;">${escapeHtml(n)}</div>
        <div style="font-family:Helvetica,Arial,sans-serif;font-size:12px;line-height:1.5;color:${SOFT};padding-top:2px;">${escapeHtml(title)}</div>`;
  const eyebrow = (text: string) =>
    `<div style="font-family:Helvetica,Arial,sans-serif;font-size:10px;letter-spacing:2.5px;text-transform:uppercase;color:${GOLD};font-weight:bold;margin:26px 0 12px 0;">${text}</div>`;
...
<meta name="x-apple-disable-message-reformatting">
<meta name="color-scheme" content="light">
<title>Your seat &middot; 2026 Lurie Children&rsquo;s &amp; AALB Conference</title>
<!--[if mso]><noscript><xml><o:OfficeDocumentSettings><o:PixelsPerInch>96</o:PixelsPerInch></o:OfficeDocumentSettings></xml></noscript><![endif]-->
<style>
  body{margin:0;padding:0;}
  a{color:${LINK};}
...
<div style="display:none;max-height:0;overflow:hidden;mso-hide:all;font-size:1px;line-height:1px;color:#ECE6D7;">Your seat at the conference is booked. Please confirm a few practical details.</div>
...
<tr><td align="center" style="padding:34px 14px 44px 14px;">
  ...
  <table role="presentation" class="sl-card" width="600" cellpadding="0" cellspacing="0" border="0" style="width:600px;max-width:600px;background-color:#FBF8F1;border:1px solid #E4DAC4;box-shadow:0 18px 48px rgba(12,59,75,0.18);">
    ...
    <tr><td align="center" bgcolor="${TEAL_DEEP}" class="sl-head" style="background-color:${TEAL_DEEP};background-image:linear-gradient(160deg,${TEAL} 0%,${TEAL_DEEP} 100%);padding:44px 40px 34px 40px;">
      <div style="font-family:Helvetica,Arial,sans-serif;font-size:11px;line-height:18px;letter-spacing:4px;text-transform:uppercase;color:${GOLD_SOFT};font-weight:bold;">Lurie Children&rsquo;s &middot; AALB</div>
      <div style="font-family:Helvetica,Arial,sans-serif;font-size:9px;line-height:16px;letter-spacing:3px;text-transform:uppercase;color:#7FA7B1;padding-top:6px;">Your Seat Is Reserved</div>
      <div style="font-family:Helvetica,Arial,sans-serif;font-size:8px;line-height:10px;letter-spacing:4px;text-transform:uppercase;color:${GOLD};font-weight:bold;padding:22px 0 8px 0;">&middot;&nbsp;Second Joint Conference&nbsp;&middot;</div>
    ...
    <tr><td class="sl-body" style="padding:40px 52px 36px 52px;background-color:#FBF8F1;">
      ...
      <div style="padding:20px 0 18px 0;">
        <div style="font-family:Georgia,'Times New Roman',serif;font-size:17px;line-height:1.4;color:${INK};font-weight:bold;">${escapeHtml(name)}</div>
        ${talkTitle ? `<div style="font-family:Georgia,'Times New Roman',serif;font-style:italic;font-size:14px;line-height:1.5;color:${SOFT};padding-top:2px;">${escapeHtml(talkTitle)}</div>` : ""}
      </div>
      <table role="presentation" cellpadding="0" cellspacing="0" border="0"><tr><td style="width:46px;height:2px;background-color:${GOLD};font-size:0;line-height:0;">&nbsp;</td></tr></table>
      <div style="height:22px;line-height:22px;font-size:0;">&nbsp;</div>
      ...
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:6px 0 22px 0;">
        <tr><td bgcolor="#F2F6F6" style="background-color:#F2F6F6;border:1px solid #D9E6E8;border-left:3px solid ${TEAL};border-radius:10px;padding:16px 20px;">
          <div style="font-family:Georgia,'Times New Roman',serif;font-size:15px;line-height:1.75;color:${INK};">
            Your ticket is complimentary as a presenter, for both August 15 and 16. If a colleague is joining you, they register separately at the standard rate.
          </div>
        </td></tr>
      </table>
      ...
      ${hasDetails
        ? p(`Your page already has the dietary and accessibility notes you gave us with your proposal. Please open it and check they are still right, and add anything we are missing. It takes a minute, and it is what we use for catering, seating and accommodations on the day.`, 20)
        : p(`Please open your page and let us know about any dietary needs or accessibility accommodations. It takes a minute, and it is what we use for catering, seating and accommodations on the day.`, 20)}
      ...
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:20px 0 2px 0;">
        <tr><td align="center">
          ...
          <table role="presentation" class="sl-cta" cellpadding="0" cellspacing="0" border="0" style="display:inline-block;vertical-align:middle;margin:6px;"><tr>
            <td align="center" bgcolor="${TEAL}" style="background-color:${TEAL};border-radius:9px;">
              <a href="${portalUrl}" style="display:inline-block;padding:15px 30px;font-family:Helvetica,Arial,sans-serif;font-size:14px;font-weight:bold;letter-spacing:0.4px;color:#ffffff;text-decoration:none;border-radius:9px;">${hasDetails ? "Check my details" : "Add my details"} &nbsp;&rarr;</a>
            </td>
          </tr></table>
          <table role="presentation" class="sl-cta" cellpadding="0" cellspacing="0" border="0" style="display:inline-block;vertical-align:middle;margin:6px;"><tr>
            <td align="center" bgcolor="#FBF8F1" style="background-color:#FBF8F1;border:1.5px solid ${GOLD};border-radius:9px;">
      ...
      ${p(`Anything at all about your session, just reply to this letter.`, 22)}
...
// Payment reminder for an accepted sponsor or exhibitor who has not paid.
// Deliberately gracious: they said yes, and in the exhibitor case they have
// often already sent a logo and their table details, so the letter thanks
// them for what is in and names payment as the one step left. Same engraved
// stationery as the rest of the sponsor correspondence.
export function sponsorPaymentReminderEmail({
  contactName,
  companyName,
  tierName,
  amountLabel,
  ...
  amountLabel: string;
  payUrl: string;
  // They already sent artwork, so the letter should acknowledge it rather
  // than read as though nothing has happened.
  hasLogo: boolean;
  // 1 for the first nudge; later ones get a slightly firmer opening.
...
<meta name="x-apple-disable-message-reformatting">
<meta name="color-scheme" content="light">
<title>Completing your ${escapeHtml(tierName)} &middot; 2026 Conference</title>
<!--[if mso]><noscript><xml><o:OfficeDocumentSettings><o:PixelsPerInch>96</o:PixelsPerInch></o:OfficeDocumentSettings></xml></noscript><![endif]-->
<style>
  body{margin:0;padding:0;}
  a{color:${LINK};}
...
<tr><td align="center" style="padding:34px 14px 44px 14px;">
  ...
  <table role="presentation" class="sl-card" width="600" cellpadding="0" cellspacing="0" border="0" style="width:600px;max-width:600px;background-color:#FBF8F1;border:1px solid #E4DAC4;box-shadow:0 18px 48px rgba(12,59,75,0.18);">
    ...
    <tr><td align="center" bgcolor="${TEAL_DEEP}" class="sl-head" style="background-color:${TEAL_DEEP};background-image:linear-gradient(160deg,${TEAL} 0%,${TEAL_DEEP} 100%);padding:44px 40px 34px 40px;">
      <div style="font-family:Helvetica,Arial,sans-serif;font-size:11px;line-height:18px;letter-spacing:4px;text-transform:uppercase;color:${GOLD_SOFT};font-weight:bold;">Lurie Children&rsquo;s &middot; AALB</div>
      <div style="font-family:Helvetica,Arial,sans-serif;font-size:9px;line-height:16px;letter-spacing:3px;text-transform:uppercase;color:#7FA7B1;padding-top:6px;">One Step Remaining</div>
      <div style="font-family:Helvetica,Arial,sans-serif;font-size:8px;line-height:10px;letter-spacing:4px;text-transform:uppercase;color:${GOLD};font-weight:bold;padding:22px 0 8px 0;">&middot;&nbsp;Second Joint Conference&nbsp;&middot;</div>
    ...
    <tr><td class="sl-body" style="padding:40px 52px 36px 52px;background-color:#FBF8F1;">
      ...
      <div style="padding:20px 0 18px 0;">
        <div style="font-family:Georgia,'Times New Roman',serif;font-size:17px;line-height:1.4;color:${INK};font-weight:bold;">${escapeHtml(companyName)}</div>
        <div style="font-family:Georgia,'Times New Roman',serif;font-style:italic;font-size:14px;line-height:1.5;color:${SOFT};padding-top:2px;">${escapeHtml(tierName)} &middot; awaiting payment</div>
      </div>
      <table role="presentation" cellpadding="0" cellspacing="0" border="0"><tr><td style="width:46px;height:2px;background-color:${GOLD};font-size:0;line-height:0;">&nbsp;</td></tr></table>
      <div style="height:22px;line-height:22px;font-size:0;">&nbsp;</div>
      ...
      <p style="margin:0 0 18px 0;font-family:Georgia,'Times New Roman',serif;font-size:15.5px;line-height:1.85;color:${INK};">
        <span class="sl-dropcap" style="float:left;font-family:Georgia,'Times New Roman',serif;font-size:54px;line-height:42px;color:${TEAL};padding:6px 11px 0 0;">${later ? "W" : "T"}</span>${later
          ? `e are getting close to the conference, and I did not want <strong>${escapeHtml(companyName)}</strong> to lose its place. Everything is ready at our end; the one thing still outstanding is payment.`
          : `hank you again for coming on board. ${hasLogo ? `We have your logo and your table details safely on file, and` : `We have your details on file, and`} there is just one step left to confirm <strong>${escapeHtml(companyName)}</strong>.`}
      </p>

      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:6px 0 22px 0;">
        <tr><td bgcolor="#FBF4E2" style="background-color:#FBF4E2;border:1px solid #EAD9AE;border-radius:10px;padding:18px 22px;">
          <div style="font-family:Helvetica,Arial,sans-serif;font-size:10px;letter-spacing:2.5px;text-transform:uppercase;color:${GOLD};font-weight:bold;padding-bottom:6px;">Outstanding</div>
          <div style="font-family:Georgia,'Times New Roman',serif;font-size:26px;line-height:1.2;color:#3C2E10;font-weight:bold;">${escapeHtml(amountLabel)}</div>
          <div style="font-family:Georgia,'Times New Roman',serif;font-size:14px;line-height:1.6;color:#6B5316;padding-top:6px;">${escapeHtml(tierName)}</div>
        </td></tr>
      </table>

      ${p(`Your place is not confirmed until this is settled, and space is limited, so I would rather chase you than quietly give it away. You can pay by card in a minute on your sponsor page.`, 20)}
      ...
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:20px 0 2px 0;">
        <tr><td align="center">
          ...
          <table role="presentation" class="sl-cta" cellpadding="0" cellspacing="0" border="0" style="display:inline-block;vertical-align:middle;margin:6px;"><tr>
            <td align="center" bgcolor="${TEAL}" style="background-color:${TEAL};border-radius:9px;">
              <a href="${payUrl}" style="display:inline-block;padding:15px 30px;font-family:Helvetica,Arial,sans-serif;font-size:14px;font-weight:bold;letter-spacing:0.4px;color:#ffffff;text-decoration:none;border-radius:9px;">Complete payment &nbsp;&rarr;</a>
            </td>
          </tr></table>
          <table role="presentation" class="sl-cta" cellpadding="0" cellspacing="0" border="0" style="display:inline-block;vertical-align:middle;margin:6px;"><tr>
            <td align="center" bgcolor="#FBF8F1" style="background-color:#FBF8F1;border:1.5px solid ${GOLD};border-radius:9px;">
      ...
      ${p(`If something has changed, or a purchase order or invoice would make this easier at your end, just reply and tell me. We would far rather sort it out than lose you.`, 22)}
